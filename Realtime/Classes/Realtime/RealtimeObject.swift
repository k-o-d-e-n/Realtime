...
protocol KeyedRealtimeValue: RealtimeValue, Hashable {
...
extension Hashable where Self: RealtimeValue {
...
protocol ChangeableRealtimeEntity: ChangeableRealtimeValue {
...
protocol RealtimeEntityActions: RealtimeValueActions {
...
class _RealtimeValue: ChangeableRealtimeValue, RealtimeValueActions, KeyedRealtimeValue {
    var uniqueKey: String { return dbKey }
    private(set) var dbRef: DatabaseReference
    private var observingToken: UInt?
    internal var localValue: Any? { return nil }
    internal required init(dbRef: DatabaseReference) {
        self.dbRef = dbRef
    }
    ...
    @discardableResult
    func save(completion: ((Error?, DatabaseReference) -> ())? = nil) -> Self {
        RemoteManager.save(entity: self, completion: completion)
        return self
    }

    @discardableResult
    func save(with values: [Database.UpdateItem], completion: ((Error?, DatabaseReference) -> ())? = nil) -> Self {
    ...
    @discardableResult
    func remove(completion: ((Error?, DatabaseReference) -> ())? = nil) -> Self {
    ...
    @discardableResult
    func remove(with linkedRefs: [DatabaseReference], completion: Database.TransactionCompletion? = nil) -> Self {
    ...
    internal func didRemove() { }
    internal func didSave() { }
    
    // MARK: Changeable
    
    var hasChanges: Bool { return false }

    // MARK: Realtime Value

    internal required init(snapshot: DataSnapshot) {
        dbRef = snapshot.ref
...
class _RealtimeEntity: _RealtimeValue, ChangeableRealtimeEntity, RealtimeEntityActions {
    /// Warning! Values should be only on first level in hierarchy, else other data is lost.
    @discardableResult
    func update(completion: ((Error?, DatabaseReference) -> ())? = nil) -> Self {
    ...
    @discardableResult
    func updateThoroughly(completion: Database.TransactionCompletion?) -> Self {
    ...
    @discardableResult
    func update(with values: [Database.UpdateItem], completion: ((Error?, DatabaseReference) -> ())?) -> Self {
    ...
    @discardableResult
    func update(with values: [String : Any?], completion: ((Error?, DatabaseReference) -> ())?) -> Self {
        RemoteManager.update(entity: self, with: values, completion: completion)

        return self
    }
    func insertChanges(to values: inout [String : Any?], keyed from: DatabaseReference) { fatalError("You should override this method " + #function) }
    func insertChanges(to values: inout [Database.UpdateItem]) { fatalError("You should override this method " + #function) }
    func insertChanges(to transaction: RealtimeTransaction) { fatalError("You should override this method " + #function) }
...
class RealtimeObject: _RealtimeEntity {
    typealias ChangeableProperty = ChangeableRealtimeEntity
    typealias Property = RealtimeValue
    ...
    override var hasChanges: Bool { return _changeProps.first { $0.hasChanges } != nil }
//    var localChanges: Any? { return keyedValues { return $0.localChanges } }
    override internal var localValue: Any? { return keyedValues { return $0.localValue } }
...
//    lazy var parent: RealtimeObject? = self.dbRef.parent.map(RealtimeObject.init) // should be typed
    ...
    @discardableResult
    override func updateThoroughly(completion: ((Error?, DatabaseReference) -> ())? = nil) -> Self {
        var changes = [String: Any?]()
    ...
    @discardableResult
    override func remove(completion: ((Error?, DatabaseReference) -> ())?) -> Self {
...
//        }
    ...
    override func insertChanges(to values: inout [String : Any?], keyed from: DatabaseReference) {
        _changeProps.forEach { prop in
            prop.insertChanges(to: &values, keyed: from)
        }
    }
    override func insertChanges(to values: inout [Database.UpdateItem]) {
        _changeProps.forEach { $0.insertChanges(to: &values) }
    }
    override func insertChanges(to transaction: RealtimeTransaction) {
    ...
    func register<T: ChangeableProperty>(prop: T, completion: ((T) -> Void)? = nil) -> T {
        _changeProps.append(prop)
        completion?(prop)
        
        return prop
    }
    func register<T: Property>(prop: T, completion: ((T) -> Void)? = nil) -> T {
        _props.append(prop)
        completion?(prop)
    ...
    override var debugDescription: String { return _allProps.reduce("\n{\n\tref: \(dbRef.pathFromRoot);") { $0 + "\n\"\($1.dbKey)\":" + $1.debugDescription } + "\n}" }
}

extension RealtimeObject: Linkable {
    var linksRef: DatabaseReference { return links.dbRef }
    @discardableResult
    func add(link: RealtimeLink) -> Self {
    ...
    @discardableResult
    func remove(linkBy id: String) -> Self {
        links.changeLocalValue { (values) in

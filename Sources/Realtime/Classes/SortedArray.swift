...
extension SortedArray: ExpressibleBySequence where Element: Comparable {
    public typealias SequenceElement = Element
    public init<S>(_ sequence: S) where S : Sequence, SortedArray.SequenceElement == S.Element {
        self.init(unsorted: sequence)
    }
}
